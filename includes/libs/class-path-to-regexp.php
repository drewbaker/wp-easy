<?php
/**
 * Class file for PathToRegexp lib.
 *
 * @package WpEasy
 */

namespace WpEasy\Libs;

/**
 * PathToRegexp class.
 */
class Path_To_Regexp {

	private static function get_regexp_source( $regexp ) {
		$delimiter         = substr( $regexp, 0, 1 );
		$end_delimiter_pos = strrpos( $regexp, $delimiter );
		$source            = substr( $regexp, 1, $end_delimiter_pos - 1 );
		return $source;
	}
	...
	*/
	public static function convert( $path, &$keys = array(), $options = array() ) {
		...
		if ( is_array( $path ) ) {
			...
			$path = array_map(
				function( $value ) use ( &$keys, &$options ) {
					return self::get_regexp_source( self::convert( $value, $keys, $options ) );
				},
				$path
			);
		...
		$path_regexp  = '/' . implode( '|', $path_regexps ) . '/';

		// Alter the path string into a usable regexp.
		$path = preg_replace_callback(
			$path_regexp,
			function( $matches ) use ( &$keys, &$index ) {
				if ( count( $matches ) > 1 ) {
					$escaped = $matches[1];
				}
				if ( count( $matches ) > 2 ) {
	...
	public static function match( $regexp, $route ) {
		...
		if ( count( $matches ) == 0 ) {
			...
			$are_values_null = true;
			foreach ( $matches as $key => $match ) {
				if ( ! empty( $match ) ) {
					$matches[ $key ] = $match[0];
					$are_values_null = false;
				} else {
					$matches[ $key ] = null;
				}
			}
